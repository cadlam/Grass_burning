...
yr2_data$plot <- as.factor(yr2_data$plot)
yr2_data$subplot <- as.factor(yr2_data$subplot)
yr2_data$year <- as.factor(yr2_data$year)
...
burn_model = lmer(Growth ~ planting*burn + block + (1|mainplot) + block:planting, yr2_data)
```

## Diagnostic plots
```{r}
# Interaction plot
ggplot(yr2_data, aes(x=block,y=Growth)) + geom_line(aes(group = interaction(planting,burn),color = interaction(planting,burn)))
...
planting_data <- aggregate(Growth~planting+block,yr2_data,mean)
str(planting_data)
## Model with planting treatments averaged:
planting_avg_model <- lm(Growth~planting+block, planting_data)
...
burn_data <- aggregate(Growth~mainplot+burn+block,yr2_data,mean)
str(burn_data)

# Model with burn treatments averaged (straightforward RCBD with block as fixed):
burn_avg_model_fixed <- lm(Growth~planting + block, planting_data)
